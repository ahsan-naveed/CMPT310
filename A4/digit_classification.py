...
def derivative_activation(x):
    def sig(y): return np.exp(y) / (1 + np.exp(y))
...
def backpropagation(x, t, W, B):
    ...
    # from the algorithm discussed in class t = y & z_s = inputs

    # initialization
    delta = derivative_activation(z_s[-1]) * (t - a_s[-1])
    d_b[-1] = -delta
    d_w[-1] = np.dot(delta, a_s[-2].transpose())

    # computing delta's with respect to biases and weights
    for i in range(2, len(W) + 1):
        z = z_s[-i]
        delta = np.dot(W[-i+1].transpose(), delta) * derivative_activation(z)
        d_b[-i] = -delta
        d_w[-i] = np.dot(delta, a_s[-i-1].transpose())
...
def test(model_path):
    ...
    # Print Accuracy
    print("Test Error: {0:4.4f}\tTest Acc: {1:4.4f}".format(
        test_loss, test_acc))
