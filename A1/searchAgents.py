...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState, costFn=lambda x, y: 1, goal=(1, 1), start=None, warn=True, visualize=True):
        """
    ...
    def getStartState(self):
        ...
        # state is componsed  of the starting Pac-Man position and the
        # list containing the four corners.

        # initially no corners are visited therefore list of visted corners is empty
        # tuple(0) = starting position
        # tuple(1) = list of visited corners
    ...
    def isGoalState(self, state):
        ...
        # all four corners that we need to visit
        goalCorners = 4

        # corners that we have visited so far
        visitedCorners = state[1]

        # check if the total visited corners equals to 4
        # state [0] = current position
        # state [1] = to store total number of visited corners
        return len(visitedCorners) == goalCorners
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            x, y = currentPosition  # current node
            dx, dy = Actions.directionToVector(action)
            nextx, nexty = int(x + dx), int(y + dy)  # next node
            hitsWall = self.walls[nextx][nexty]

            "*** YOUR CODE HERE ***"

            if not hitsWall:
                totalVistedCorners = visitedCorners

                if (nextx, nexty) not in visitedCorners and (nextx, nexty) in self.corners:
                    # successor to the current node which is also one of the corners
                    totalVistedCorners = visitedCorners + [(nextx, nexty)]

                # successor[0]
                #   successor[0][0] = node
                #   successor[0][1] = visited corners
                # successor[1] = action
                # successor[2] = stepCost
                successors.append(
                    (((nextx, nexty), totalVistedCorners), action, 1))
